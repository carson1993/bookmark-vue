...
- **拖拽移动** - 拖拽书签卡片到侧边栏分类或子分类标签即可移动书签
- **链接测速** - 一键检测当前分类下所有书签的连通性和延迟
- **智能图标** - Canvas 透明度检测 + 四级降级加载 + favicon 淡入过渡
- **图标缓存** - 成功加载的 favicon 自动缓存到 localStorage，减少网络消耗
- **固定布局** - Header、Footer、侧边栏、滚动按钮固定不动，仅书签区滚动
- **滚动条美化** - 自定义皮卡丘主题滚动条，与玻璃拟态风格统一
- **滚动按钮** - 右侧置顶/置底按钮，到达边界自动隐藏
